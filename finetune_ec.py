    ...
    for train_data in data_loader:
        ...
        for i in range(args.iter+1):
            ...
            if i % 1000 == 0:
                ...
                num_mlp = sum(p.numel() for p in model.net_coarse.parameters() if p.requires_grad)
                storage = string_mb + num_mlp*2*4/(10**6) + (args.trank * 32 * 2 * 4)/(10**6) # vector size
                ...
                existing_csv_file_path = f"{i}_ec.csv"
